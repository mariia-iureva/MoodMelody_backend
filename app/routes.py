...
def store_tokens_in_db(session_id, token_info):
    ...
    user = User(session_id=session_id, access_token=access_token, refresh_token=refresh_token)
    db.session.add(user)
    db.session.commit()
...
def recommend():
    ...
    if not token_info:
        return jsonify({"authorized": False, "message": "User not authorized, please log in to Spotify.", "auth_url": url_for('main.login', session_id=session_id, _external=True)})
